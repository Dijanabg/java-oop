...
public class Main {
	...
	public static void main(String[] args) {
		 ...
		 * testu 2, prosečan broj poena, kao i metodama za postavljanje imena i
		 * prezimena studenta, broja poena i računanje prosečnog broja poena na
		 * testovima i štampanje uspeha studenata • Potom testirati klasu kreiranjem dva
		 * objekta u okviru glavnog programa, kojima postavljamo imena i prezimena i
		 * broj poena na testovima i za koje potom računamo i prikazujemo uspe
		 */
		// ******STUDENT******
		...
		// ******ZAPOSLENI******
		/*
		 * Kreirati klasu Zaposleni sa atributima ime (tipa String), prezime (tipa
		 * String) i koefRadnogMesta (tipa double), kao i metodama za postavljanje imena
		 * i prezimena zaposlenog i postavljanje koeficijenta radnog mesta, računanje
		 * plate po formuli plata = 3600 * koefRadnogMesta i štampanje plate zaposlenog.
		 * • Potom testirati klasu kreiranjem tri objekta u okviru glavnog programa,
		 * kojima postavljamo imena i prezimena i koeficijente radnog mesta i za koje
		 * potom računamo i prikazujemo platu
		 */
		 ...
		 * i radnog takta procesora, postavljanje kapaciteta memorije u GB, računanje
		 * indeksa performansi računara po formuli: indeksPerformansi = 10*radniTakt +
		 * kapacitetMemorije i štampanje indeksa performansi računara. • Potom testirati
		 * klasu kreiranjem pet objekta u okviru glavnog programa, kojima prvo
		 * postavljamo vrednosti atributa, a potom računamo i prikazujemo njihovu rang
		 * listu uređenu po indeksu performansi u opadajućem redosledu.
		 */
		// kreiranje objekata
	...
	// Racunar... racunari **** ovako napisano moze da se stavi bilo koji broj
	// argumenata tipa Racunar
	public static void prikaziRangListu(Poredak poredak, Racunar... racunari) {
		...
		/*
		 * Arrays.sort je metoda u Javi koja se koristi za sortiranje niza objekata ili
		 * niza primitivnih tipova podataka. Ova metoda koristi algoritam brzog
		 * sortiranja (quicksort) za objekte i algoritam za sortiranje mešanjem (merge
		 * sort) za primitivne tipove podataka. Sortiranje se vrši u mestu, što znači da
		 * se originalni niz menja, a ne vraća se novi niz.
		 */
		// Double.compare se koristi za poredjenje dva double broja.

		/*
		 * Arrays.sort(racunari, (r1, r2) -> Double.compare(r2.indeksPerformansi,
		 * r1.indeksPerformansi));
		 */
		...
		for (int i = 0; i < racunari.length; i++) {
			System.out.println("\t" + (i + 1) + ". " + racunari[i].getProcesor() + " - Indeks performansi: "
					+ racunari[i].indeksPerformansi);
		}
	}
}
